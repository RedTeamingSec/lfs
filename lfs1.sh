...
LFS_DISK="/dev/sdb"            # Disk utama (tanpa angka partisi)
LFS_PARTITION="/dev/sdb3"      # Partisi root LFS
LFS_SWAP_PARTITION="/dev/sdb2" # Partisi swap (opsional)
...
function check_disk_space() {
    local required_space=10 # Minimum 10GB untuk LFS lengkap
    local available_space=$(df -BG --output=avail / | tail -1 | tr -d 'G ')
    
    if [ "$available_space" -lt "$required_space" ]; then
        echo "Error: Ruang disk tidak mencukupi. Diperlukan minimal ${required_space}GB."
        exit 1
    fi
}

function prepare_lfs_partition() {
    echo -e "\n=== [1/12] Mempersiapkan Partisi LFS ==="
    
    # Cek jika partisi sudah ada
    if [ ! -b "$LFS_PARTITION" ]; then
        echo "Error: Partisi $LFS_PARTITION tidak ditemukan!"
        echo "Buat partisi terlebih dahulu dengan fdisk atau parted"
        exit 1
    fi
    
    # Cek jika partisi sudah terpasang
    if mount | grep -q "$LFS_PARTITION"; then
        echo "Error: Partisi $LFS_PARTITION sedang digunakan. Silakan unmount terlebih dahulu."
        exit 1
    fi
    
    # Format partisi sebagai ext4 (HATI-HATI: ini akan menghapus semua data)
    echo "Memformat $LFS_PARTITION sebagai ext4..."
    mkfs.ext4 -F "$LFS_PARTITION"
    
    # Aktifkan partisi swap jika ada
    if [ -b "$LFS_SWAP_PARTITION" ]; then
        echo "Mengaktifkan swap partition $LFS_SWAP_PARTITION..."
        mkswap "$LFS_SWAP_PARTITION"
        swapon "$LFS_SWAP_PARTITION"
    fi
    
    # Buat direktori mount dan mount partisi
    mkdir -pv "$LFS"
    mount -v -t ext4 "$LFS_PARTITION" "$LFS"
    
    # Tambahkan ke fstab
    echo "$LFS_PARTITION $LFS ext4 defaults 1 1" >> /etc/fstab
    
    if [ -b "$LFS_SWAP_PARTITION" ]; then
        echo "$LFS_SWAP_PARTITION none swap sw 0 0" >> /etc/fstab
    fi
    
    echo "Partisi LFS berhasil dipersiapkan di $LFS"
}

function setup_lfs_user() {
    echo -e "\n=== [2/12] Menyiapkan User LFS ==="
    
    # Buat group jika belum ada
    if ! grep -q "^$LFS_GROUP:" /etc/group; then
        groupadd "$LFS_GROUP"
    fi
    ...
    if ! id "$LFS_USER" &>/dev/null; then
        ...
        echo "lfsuser:lfs123" | chpasswd
        echo "User $LFS_USER dibuat dengan password 'lfs123'"
    else
        echo "User $LFS_USER sudah ada"
    fi
    
    # Set ownership untuk direktori LFS
    chown -v "$LFS_USER:$LFS_GROUP" "$LFS"
    
    # Buat direktori penting dalam LFS
    su - "$LFS_USER" -c "mkdir -pv $LFS/sources $LFS/tools"
    
    # Set permission untuk sources dan tools
    chmod -v a+wt "$LFS/sources"
    chown -v "$LFS_USER:$LFS_GROUP" "$LFS/tools"
    chmod -v 0755 "$LFS/tools"
...
function install_required_packages() {
    echo -e "\n=== [3/12] Menginstal Paket yang Diperlukan ==="
    
    apt-get update
    apt-get upgrade -y
    
    # Daftar paket yang diperlukan untuk LFS 8.4
    local required_packages=(
        binutils bison build-essential coreutils gawk git
        gcc g++ m4 make patch perl python sed tar texinfo
        wget bzip2 gzip xz-utils file
    )
    
    apt-get install -y "${required_packages[@]}"
    
    # Verifikasi versi penting
    echo -e "\n=== Verifikasi Versi Paket ==="
    ...
    echo "Perl version: $(perl -V:version)"
}

function download_lfs_packages() {
    echo -e "\n=== [4/12] Mengunduh Paket LFS 8.4 ==="
...
EOF
    ...
    su - "$LFS_USER" -c "wget --input-file=$wget_list --continue --directory-prefix=$LFS/sources"
...
prepare_lfs_partition
setup_lfs_user
install_required_packages
download_lfs_packages
prepare_lfs_environment
